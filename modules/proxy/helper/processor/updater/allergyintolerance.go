...
func UpdateReferenceAllergyIntolerance(entry *types.BundleEntry, register *types.SetReference, newPost []types.NewPost) (bool, error) {
	resource, ok := entry.Base.ResourceReal.(fhir.AllergyIntolerance)
	if !ok {
		return false, fmt.Errorf("failed to cast resource to AllergyIntolerance")
	}
