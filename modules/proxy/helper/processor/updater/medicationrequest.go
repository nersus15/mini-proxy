...
func UpdateReferenceMedicationRequest(entry *types.BundleEntry, register *types.SetReference, newPost []types.NewPost) (bool, error) {
	resource, ok := entry.Base.ResourceReal.(fhir.MedicationRequest)
	if !ok {
		return false, fmt.Errorf("failed to cast resource to MedicationRequest")
	}
