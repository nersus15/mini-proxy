...
func UpdateReferenceClinicalImpression(entry *types.BundleEntry, register *types.SetReference, newPost []types.NewPost) (bool, error) {
	resource, ok := entry.Base.ResourceReal.(fhir.ClinicalImpression)
	if !ok {
		return false, fmt.Errorf("failed to cast resource to ClinicalImpression")
	}
